...
- **Kaniko** (`gcr.io/kaniko-project/executor`) — инструмент от Google, который собирает образы **без привилегированного режима**, запускаясь из обычного контейнера. Давно знаком DevOps-инженерам, работает «из коробки» в любом кластере, включая Yandex Managed K8s. ⚠️ Репозиторий [GoogleContainerTools/kaniko](https://github.com/GoogleContainerTools/kaniko) **архивирован владельцем 3 июня 2025 года** и доступен только для чтения — проект больше не развивается.
...
| Время сборки **с кэшем** (повторный прогон) | быстрее через `--cache` (registry-кэш), но upload/download всех слоёв | кэш на локальном диске buildkitd, push только новых слоёв |
