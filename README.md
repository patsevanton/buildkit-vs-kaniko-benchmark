...
**Кэш.** Оба инструмента используют только registry-кэш: Kaniko через `--cache-repo`, BuildKit через `--import-cache`/`--export-cache type=registry`. Локальный кэш на ноде намеренно не используется — он копится на диске и требует очистки, а в эфемерных daemonless-подах без PVC не переживает прогон. Registry-кэш чистить не нужно: манифест кэша перезаписывается на каждом прогоне, а мусор подчищает garbage collection реестра. Хранясь вне пода, он переживает пересоздание и смену ноды — только так можно честно «прогреть» кэш между прогонами и уравнять условия обоих инструментов.
