...
**Вариант 1 — две таблицы, каждый пункт на отдельной строке**

**Преимущества**

| Kaniko | BuildKit |
|---|---|
| — **Работает без привилегий:** обычный контейнер без privileged, без docker.sock — подходит для managed-кластера и строгих политик безопасности. | — **Скорость:** многопоточная сборка (параллельные шаги), кэш слоёв и быстрый инкрементальный пересбор; на реальных Dockerfile часто в 2–3 раза быстрее Kaniko. |
| — **Простота:** один бинарник-джоб, огромное количество документации и примеров. | — **Богатый синтаксис:** `RUN --mount=type=cache\|secret\|ssh`, `RUN --mount=type=bind`, BuildKit-составные шаги, внешние кэши. |
| — **Можно собирать в любом кластере** — без daemon, sysctl и user-namespace. | — **Та же технология, что у `docker build`:** единый синтаксис для CI и local docker. |

**Недостатки**

| Kaniko | BuildKit |
|---|---|
| — **Скорость:** сборка идёт последовательно по слоям (параллельно слои не строятся), на тяжёлых Dockerfile заметно медленнее BuildKit. | — **Сложнее:** daemonless-джоб поднимает встроенный демон, требуется понимание `buildctl`/`buildkitd` и кэша. |
| — **Слабое кэширование на диске:** кэш по умолчанию пишется в registry (медленнее и дороже), локального кэша между прогонами нет. | — **Ресурсы:** многопоточность = большее пиковое потребление CPU/RAM, которое нужно учитывать в requests/limits. |
| — **Ограниченный синтаксис:** не поддерживает `RUN --mount=type=cache`, `RUN --mount=type=secret`, `--mount=type=ssh` и т.п. (часть — через флаги, но не вся). | — **Rootless-режим имеет нюансы:** нужны unprivileged user namespaces на нодах, `oci-worker-no-process-sandbox` и ослабленный seccomp/apparmor (`Unconfined`) на build-контейнере; если ноды не дают user namespaces — DaemonSet-воркараунд из `examples/kubernetes/sysctl-userns.privileged.yaml` в moby/buildkit. |
| — **Контекст и большие слои:** базовый образ и предыдущие слои скачиваются и разворачиваются целиком — при большом контексте это время и место. | |

**Вариант 2 — отдельный подраздел на каждый инструмент**

**Преимущества**

**Kaniko**
- **Работает без привилегий:** обычный контейнер без privileged, без docker.sock — подходит для managed-кластера и строгих политик безопасности.
- **Простота:** один бинарник-джоб, огромное количество документации и примеров.
- **Можно собирать в любом кластере** — без daemon, sysctl и user-namespace.
...
- **Богатый синтаксис:** `RUN --mount=type=cache|secret|ssh`, `RUN --mount=type=bind`, BuildKit-составные шаги, внешние кэши.
- **Та же технология, что у `docker build`:** единый синтаксис для CI и local docker.

**Недостатки**

**Kaniko**
- **Скорость:** сборка идёт последовательно по слоям (параллельно слои не строятся), на тяжёлых Dockerfile заметно медленнее BuildKit.
- **Слабое кэширование на диске:** кэш по умолчанию пишется в registry (медленнее и дороже), локального кэша между прогонами нет.
- **Ограниченный синтаксис:** не поддерживает `RUN --mount=type=cache`, `RUN --mount=type=secret`, `--mount=type=ssh` и т.п. (часть — через флаги, но не вся).
- **Контекст и большие слои:** базовый образ и предыдущие слои скачиваются и разворачиваются целиком — при большом контексте это время и место.

**BuildKit**
- **Сложнее:** daemonless-джоб поднимает встроенный демон, требуется понимание `buildctl`/`buildkitd` и кэша.
- **Ресурсы:** многопоточность = большее пиковое потребление CPU/RAM, которое нужно учитывать в requests/limits.
- **Rootless-режим имеет нюансы:** нужны unprivileged user namespaces на нодах, `oci-worker-no-process-sandbox` и ослабленный seccomp/apparmor (`Unconfined`) на build-контейнере; если ноды не дают user namespaces — DaemonSet-воркараунд из `examples/kubernetes/sysctl-userns.privileged.yaml` в moby/buildkit.

**Вариант 3 — одна таблица, столбец-инструмент**

| | Kaniko | BuildKit |
|---|---|---|
| **Преимущества** | — **Работает без привилегий:** обычный контейнер без privileged, без docker.sock — подходит для managed-кластера и строгих политик безопасности.<br>— **Простота:** один бинарник-джоб, огромное количество документации и примеров.<br>— **Можно собирать в любом кластере** — без daemon, sysctl и user-namespace. | — **Скорость:** многопоточная сборка (параллельные шаги), кэш слоёв и быстрый инкрементальный пересбор; на реальных Dockerfile часто в 2–3 раза быстрее Kaniko.<br>— **Богатый синтаксис:** `RUN --mount=type=cache\|secret\|ssh`, `RUN --mount=type=bind`, BuildKit-составные шаги, внешние кэши.<br>— **Та же технология, что у `docker build`:** единый синтаксис для CI и local docker. |
| **Недостатки** | — **Скорость:** сборка идёт последовательно по слоям (параллельно слои не строятся), на тяжёлых Dockerfile заметно медленнее BuildKit.<br>— **Слабое кэширование на диске:** кэш по умолчанию пишется в registry (медленнее и дороже), локального кэша между прогонами нет.<br>— **Ограниченный синтаксис:** не поддерживает `RUN --mount=type=cache`, `RUN --mount=type=secret`, `--mount=type=ssh` и т.п. (часть — через флаги, но не вся).<br>— **Контекст и большие слои:** базовый образ и предыдущие слои скачиваются и разворачиваются целиком — при большом контексте это время и место. | — **Сложнее:** daemonless-джоб поднимает встроенный демон, требуется понимание `buildctl`/`buildkitd` и кэша.<br>— **Ресурсы:** многопоточность = большее пиковое потребление CPU/RAM, которое нужно учитывать в requests/limits.<br>— **Rootless-режим имеет нюансы:** нужны unprivileged user namespaces на нодах, `oci-worker-no-process-sandbox` и ослабленный seccomp/apparmor (`Unconfined`) на build-контейнере; если ноды не дают user namespaces — DaemonSet-воркараунд из `examples/kubernetes/sysctl-userns.privileged.yaml` в moby/buildkit. |
