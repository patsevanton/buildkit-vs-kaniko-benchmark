...
#
# Зачем: пара kaniko+buildkit одного проекта должна стартовать одновременно,
# иначе две линии на дашборде Grafana (buildkit и kaniko) начинаются в разное
# время. Раннер с concurrent=2 (gitlab-runner/values.yaml) при занятой паре
# ставит джобы нового пайплайна в очередь, и пара разъезжается по времени.
#
# Использование:
